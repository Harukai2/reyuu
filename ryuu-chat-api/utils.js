...
function getHeaders(url, options, ctx, customHeader) {
    ...
    if (ctx && ctx.i_userID) {
    headers["X-MSGR-User"] = ctx.i_userID;  // Use i_userID for secondary profile
} else if (ctx && ctx.userID) {
    headers["X-MSGR-User"] = ctx.userID;    // Fallback to userID for main profile
		}
    ...
    return (
        ...
        presenceEncode(
            JSON.stringify({
                ...
                user: ctx.i_userID || userID,  // Use i_userID if available
                state: {
                    ut: 0,
                    t2: [],
                    lm2: null,
                ...
                ch: {
                    ["p_" + (ctx.i_userID || userID)]: 0  // Use i_userID for presence tracking
                }
            })
        )
    );
